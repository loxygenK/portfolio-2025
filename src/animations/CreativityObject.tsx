...
import { forXY, randomInt, XY } from "../sketch/util";
...
export const CreativityObject: FC<Props> = ({ idPrefix, gridAmount, canvasArea, length }) => {
  return (
    <g id={idPrefix} className={styles.root}>
      {Array.from({ length: gridAmount[1] }).map((_, y) => (
        Array.from({ length: gridAmount[0] }).map((_, x) => (
          <Shapes grid={[x, y]} idPrefix={idPrefix} gridAmount={gridAmount} canvasArea={canvasArea} length={length} />
        ))
      ))}
      <Line
        origin={[0, 0]}
        points={[[1200, 360], [2000, 360]]}
        fill="none"
        stroke="#fff"
        strokeWidth={1}
...
const Shapes: FC<ShapesProps> = ({ grid, idPrefix, canvasArea, gridAmount }) => {
  ...
  const skew = useMemo(() => Array.from({ length: pointCount }).map(() => [
    randomInt(-1, 1),
    randomInt(-1, 1),
  ]), [])

  const points = useMemo(() => generatePolygon(pointCount, LINE_LENGTH).map(([x, y], i) => [
    x + skew[i][0],
    y + skew[i][1],
  ] as const), []);
  ...
  return (
    <>
      ...
      <Line
        origin={[0, 0]}
        points={[center, [1150, 360 + (grid[1] - gridAmount[1] / 2) * (2 * gridAmount[0]) + (grid[0] - gridAmount[0] / 2) * 2]]}
