...
const LOGICAL_THINGS = [
  "RULES", "LAWS", "BINARIES", "BOOLEAN", "STRUCTURE",
];
const CREATIVE_THINGS = [
  "IDEAS", "AMBIGUITY", "FEEL", "ADLIB", "VIBES",
];

export const Message: SubAnimationFC = ({ registerAnimation, hidden }) => {
  useSubAnimation(
    registerAnimation,
    () => createTimeline()
      .add(`.${styles.logic}`, {
        opacity: [
          {
            from: "0",
            to: stagger(-0.3, { start: 1, reversed: true, modifier: (value) => Math.max(value, 0.1) }),
            duration: 0,
            delay: stagger(80),
          },
          {
      ...
      .add(svg.createDrawable("#msgline-1"), {
        draw: { from: "0 0.65", to: "0 0.7", },
        duration: 500,
        ease: "linear",
        frameRate: 10,
      }, 700)
      .add([svg.createDrawable("#msgline-2"), svg.createDrawable("#msgline-3")], {
        draw: { from: "0 0.12", to: "0 0.14", },
        duration: 500,
        ease: "linear",
        frameRate: 10,
      }, 700)
      .add(`.${styles.creativity}`, {
        opacity: [
          {
            from: "0",
      ...
      .add(`.${styles.creativeThings} > span`, {
      ...
      .add(`#msgshapes`, {
        ...
        rotate: { from: "15deg", to: (_, i) => i % 4 == 0 ? "-=15deg" : "+=15deg", },
        frameRate: 10,
      }, 1900)
      .add(`.${styles.phrase}`, {
      ...
      .add(`.${styles.phrase} > span > span`, {
        x: [
          { from: "0.2em", to: 0, duration: 1000, delay: stagger(300) }
        ],
        opacity: [
          { to: 1, duration: 100, delay: stagger(300), },
        ],
        frameRate: 10,
      }, 2400)
  );
  ...
  return (
    <div className={styles.root} style={{ display: hidden ? "none" : undefined }}>
      <div className={styles.logicTexts}>
        {Array.from({ length: 6 }).map(() => (
          <span className={styles.logic}>
            LOGIC
          </span>
        ))}
      </div>
      <div className={styles.logicalThings}>
        <span>
          {(LOGICAL_THINGS.map(x => x + " ・ ")).join("").repeat(5)}
        </span>
      </div>
      ...
      <div className={styles.creativeThings}>
        <span>
          {(CREATIVE_THINGS.map(x => x + " ・ ")).join("").repeat(5)}
        </span>
      </div>
      ...
      <Sketch>
        ...
        <g className={styles.shapes}>
          <CreativityObject idPrefix="msg" gridAmount={[7, 6]} canvasArea={[800, 600]} length={20} />
        </g>
      </Sketch>
    </div>
  );
