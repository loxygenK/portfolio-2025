...
import styles from "./IntersectScene.module.css";
import { angled, cssXY, offXY, randomInt, type XY } from "../../sketch/util";
...
export const IntersectScene: SubAnimationFC = ({
  registerAnimation,
  hidden,
}) => {
  const angleSkews = useMemo(
    () => Array.from({ length: AMOUNT }).map(() => randomInt(-5, 5)),
    [],
  );
  const lengthSkews = useMemo(
    () => Array.from({ length: AMOUNT }).map(() => randomInt(-20, 20)),
    [],
  );
  ...
  useSubAnimation(registerAnimation, () =>
    createTimeline()
      ...
      .add(
        ["#mainline-1", "#mainline-2", "#mainline-3"],
        {
          stroke: {
            from: "#fff3",
            to: "#0003",
  ...
  return (
    <Sketch className={hidden ? styles.hidden : ""} id="meet-root">
      <g id="viewportRoot" className={styles.viewportRoot}>
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <Line
            ...
            key={i}
            style={{
              transformOrigin: cssXY(
                offXY(
                  [PRISM_X + 50, PRISM_Y + 30],
                  [5 - 0.7 * i, -2 + 0.5 * i],
                ),
              ),
            }}
      ...
      <linearGradient id="entryRayFill">
        <stop offset="0" stopColor="#FF00D9" />
        <stop offset="0.355769" stopColor="#FF7B00" />
        <stop offset="0.653846" stopColor="#91FF00" />
